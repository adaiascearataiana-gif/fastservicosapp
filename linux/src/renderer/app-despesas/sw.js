/* FAST Servicos - app Despesas - Service Worker persistente
   (r134; r144 = novos icones; r145 = SHARE TARGET unico arquivo;
    r403 (2.3.32) = SHARE TARGET MULTI-ARQUIVO: o app Despesas aparece na
   ...
   r403 SHARE TARGET (como funciona):
   ...
   2. O SW extrai TODOS os arquivos do formData, guarda
      ...
      (fast-despesas-share-r403) e responde 303 para "./?share=1".
...
const CACHE='fast-despesas-r403';
const CORE=['./','./index.html','./manifest.webmanifest','../assets/app-despesas-192.png?v=r403','../assets/app-despesas-512.png?v=r403'];
const SHARE_CACHE='fast-despesas-share-r403';
    ...
    event.respondWith(
      lerShare().then(function(pacote){
        ...
        if(pacote&&pacote.files&&pacote.files.length){
          /* formato novo r403 (lista) + campo "item" de compat r145 */
