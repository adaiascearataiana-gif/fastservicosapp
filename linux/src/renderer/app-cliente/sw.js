/* FAST Servicos - app Cliente FAST - Service Worker persistente (r134)
   ...
   Modelo network-first (igual ao r118 que sempre funcionou):
   - recursos: rede primeiro (cache:'no-store'); cache so como fallback
     quando a rede falha (offline).
   - activate: apaga caches antigos (cliente-fast-r118 e anteriores).
   - SEM self.unregister e SEM clients.navigate (nada de reload em loop). */
const C='cliente-fast-r134',A=['./','./index.html','./manifest.webmanifest','../assets/fast-servicos-icon-192.png','../assets/fast-servicos-icon-512.png'];
