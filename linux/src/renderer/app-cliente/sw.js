/* FAST Servicos - app Cliente FAST - Service Worker persistente (r249)
   ...
   Modelo network-first (igual ao r249 que sempre funcionou):
   - recursos: rede primeiro (cache:'no-store'); cache so como fallback
     quando a rede falha (offline).
   - activate: apaga caches antigos (cliente-fast-r249 e anteriores).
   - SEM self.unregister e SEM clients.navigate (nada de reload em loop). */
const C='cliente-fast-r249',A=['./','./index.html','./manifest.webmanifest','../assets/fast-servicos-icon-192.png','../assets/fast-servicos-icon-512.png'];
