/* FAST Servicos - app Cliente FAST - Service Worker persistente (r2421)
   ...
   Modelo network-first (igual ao r2421 que sempre funcionou):
   - recursos: rede primeiro (cache:'no-store'); cache so como fallback
     quando a rede falha (offline).
   - activate: apaga caches antigos (cliente-fast-r2421 e anteriores).
   - SEM self.unregister e SEM clients.navigate (nada de reload em loop). */
const C='cliente-fast-r2421',A=['./','./index.html','./manifest.webmanifest','../assets/fast-servicos-icon-192.png','../assets/fast-servicos-icon-512.png'];
